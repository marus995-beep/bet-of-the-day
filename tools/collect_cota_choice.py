...
This is the one genuinely interactive tool in the pipeline — it blocks (using
Telegram's own long-polling, not a tight loop) until the user taps a preset
button or types a manual value, or until --max-wait-seconds elapses. Presets
are 5x/10x/15x/25x; manual entry accepts anything in (1, 100]  — a manual
value over 100 is rejected with an explanation, and the user is re-prompted.
