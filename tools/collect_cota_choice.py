...
inline-keyboard Telegram message, and wait for their real response.

Blocks (using Telegram's own long-polling, not a tight loop) until the user
taps a preset button or types a manual value, or until --max-wait-seconds
elapses. Presets are 5x/10x/15x/25x; manual entry accepts anything in
(1, 100] — a manual value over 100 is rejected with an explanation, and the
user is re-prompted. If the deadline passes with no valid response, this
exits successfully (not an error) with "chosen_cota": null and
"source": "none", so the caller can fall back to prepare_bet_slip.py's own
10x default and proceed immediately — the whole point of a long
--max-wait-seconds is that the caller acts the moment a response arrives,
not on a fixed later schedule.

Usage:
    python tools/collect_cota_choice.py --chat-id 880474256 --date 2026-08-17 \
        [--max-wait-seconds 7200] [--output .tmp/cota_choice_2026-08-17.json]
"""
...
def write_output(args, result):
    ...
    if result["chosen_cota"] is None:
        print(f"No chosen cotă (source: {result['source']})")
    else:
...
def main():
    ...
    warn(f"No cotă choice received within {args.max_wait_seconds}s — reporting 'none' so the caller can use its default.")
